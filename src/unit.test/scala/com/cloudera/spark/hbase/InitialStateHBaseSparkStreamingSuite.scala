...
class InitialStateHBaseSparkStreamingSuite extends FunSuite with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  test("test for Hbase Receiver") {
    ...
    val dstream = ssc.receiverStream(new HbaseReceiver(config, "t1", new Scan(),
      (result: Result) => {
        if (result.list() != null) {
    ...
    val hbaseContext = new HBaseContext(sc, config);
    counts.foreachRDD(rdd =>hbaseContext.bulkPut[Long](rdd,
      tableName,
      (putRecord) => {
        val put = new Put(Bytes.toBytes("-" + counts))

        put.add(Bytes.toBytes(columnFamily), Bytes.toBytes("-" + counts), Bytes.toBytes("-" + counts))
        put
